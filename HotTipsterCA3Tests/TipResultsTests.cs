...
namespace HotTipsterCA3.Tests
{
    ...
    public class FileAccessTests
    {
        ...
        public void WriteResultsCollectionToFile()
        {
            TipResults tr = new TipResults();            
            AddFirstResult(tr);
            AddRemainingResults(tr);
        ...
        {            
            ...
            Assert.AreEqual(true, fa.FileExists(System.Configuration.ConfigurationManager.AppSettings["fileName"]));
        }

        [TestMethod()]
        public void ReadResultsCollectionFromFileTest()
        {            
            WriteResultsCollectionToFile();
            AddFirstResult(Results);
            AddRemainingResults(Results);
            TipResults fromFile = fa.ReadResultsCollectionFromFile(System.Configuration.ConfigurationManager.AppSettings["fileName"]);
            foreach (TipResult t in Results)
            {
                if(!fromFile.Contains(t))
                {
                    Assert.Fail("Not all results expected to be in file were found.");
                }
            }
        }
        ...
        [TestMethod()]
        public void WriteResultsCollectionToFileTest()
        {
            AddFirstResult(Results);
            Assert.AreEqual(true, fa.WriteResultsCollectionToFile(System.Configuration.ConfigurationManager.AppSettings["fileName"], Results));
            AddRemainingResults(Results);
            Assert.AreEqual(true, fa.WriteResultsCollectionToFile(System.Configuration.ConfigurationManager.AppSettings["fileName"], Results));
        }
